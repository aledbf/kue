...
function getSearch() {
  ...
  return search = reds.createSearch(redis.prefix() + ':search');
};
...
exports.range = function(from, to, order, fn){
  redis.client().zrange(redis.prefix() + ':jobs', from, to, get(fn, order));
};
...
exports.rangeByState = function(state, from, to, order, fn){
  redis.client().zrange(redis.prefix() + ':jobs:' + state, from, to, get(fn, order));
};
...
exports.rangeByType = function(type, state, from, to, order, fn){
  redis.client().zrange(redis.prefix() + ':jobs:' + type + ':' + state, from, to, get(fn, order));
};
...
exports.get = function(id, fn){
  ...
  job.id = id;
  client.hgetall(redis.prefix() + ':job:' + job.id, function(err, hash){
    if (err) return fn(err);
...
exports.log = function(id, fn){
  redis.client().lrange(redis.prefix() + ':job:' + id + ':log', 0, -1, fn);
};
...
Job.prototype.log = function(str){
  ...
  this.client.rpush(redis.prefix() + ':job:' + this.id + ':log', str);
...
Job.prototype.set = function(key, val, fn){
  this.client.hset(redis.prefix() + ':job:' + this.id, key, val, fn || noop);
  return this;
};
...
Job.prototype.get = function(key, fn){
  this.client.hget(redis.prefix() + ':job:' + this.id, key, fn || noop);
  return this;
};
...
Job.prototype.remove = function(fn){
  ...
  events.remove(this);
  this.client.del(redis.prefix() + ':job:' + this.id + ':log', noop);
  this.client.del(redis.prefix() + ':job:' + this.id, fn || noop);
  return this;
};
...
Job.prototype.removeState = function(fn){
  var client = this.client
    , state = this._state;
  client.zrem(redis.prefix() + ':jobs', this.id);
  client.zrem(redis.prefix() + ':jobs:' + state, this.id);
  client.zrem(redis.prefix() + ':jobs:' + this.type + ':' + state, this.id);
  return this;
};
...
Job.prototype.state = function(state){
  ...
  client.zadd(redis.prefix() + ':jobs', this._priority, this.id);
  client.zadd(redis.prefix() + ':jobs:' + state, this._priority, this.id);
  client.zadd(redis.prefix() + ':jobs:' + this.type + ':' + state, this._priority, this.id);
  // increase available jobs, used by Worker#getJob()
  if ('inactive' == state) client.lpush(redis.prefix() + ':' + this.type + ':jobs', 1);
  this.set('updated_at', Date.now());
  return this;
};
...
Job.prototype.save = function(fn){
  ...
  // incr id
  client.incr(redis.prefix() + ':ids', function(err, id){
    if (err) return fn(err);
    var key = redis.prefix() + ':job:' + id;
    self.id = id;
    self._state = self._state || 'inactive';
    if (max) client.hset(key, 'max_attempts', max);
    client.sadd(redis.prefix() + ':job:types', self.type);
