...
Queue.prototype.setting = function(name, fn){
  this.client.hget(prefix + ':settings', name, fn);
  return this;
};
  ...
  while (n--) {
    (function(worker){
      ...
      worker.on('job complete', function(job){
        self.client.incrby(prefix + ':stats:work-time', job.duration);
      });
...
Queue.prototype.types = function(fn){
  this.client.smembers(prefix + ':job:types', fn);
  return this;
};
...
Queue.prototype.state = function(state, fn){
  this.client.zrange(prefix + ':jobs:' + state, 0, -1, fn);
  return this;
};
...
Queue.prototype.workTime = function(fn){
  this.client.get(prefix + ':stats:work-time', function(err, n){
...
Queue.prototype.card = function(state, fn){
  this.client.zcard(prefix + ':jobs:' + state, fn);
  return this;
};
